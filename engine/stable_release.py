...
v8.0.0 is the Rust backend stable cleanup / production-stabilization release. These helpers are
...
STABLE_RELEASE_TARGET = "v8.0.0 Rust Backend Stable Release"
...
def compute_stable_release_check(root: str | Path | None = None) -> dict[str, Any]:
    ...
    if not version.startswith("2.150"):
        items.append(StableItem("version.rc", "Stable release version", "warn", f"VERSION is {version}; expected 2.150.x for v8 stable", "release", "Update VERSION and release notes before packaging v8.0.0 stable."))
    else:
