...
v8.1.0 is the Rust scheduler-authority stable release. These helpers are
...
STABLE_RELEASE_TARGET = "v8.1.0 Rust Scheduler Authority Stable Release"
...
def compute_stable_release_check(root: str | Path | None = None) -> dict[str, Any]:
    ...
    if not version.startswith("2.151"):
        items.append(StableItem("version.rc", "Stable release version", "warn", f"VERSION is {version}; expected 2.151.x for v8.1 scheduler-authority stable", "release", "Update VERSION and release notes before packaging v8.1.0 stable."))
    else:
