...
pub const OP_RUN_ROUTEROS_AUTHENTICATED_READ_FIXTURE: &str =
...
pub const OP_BUILD_ROUTEROS_SHADOW_COLLECTOR_BUNDLE: &str =
    "build-routeros-shadow-collector-bundle";
pub const OP_BUILD_ROUTEROS_LIVE_READ_SHADOW_PARITY: &str =
    "build-routeros-live-read-shadow-parity";
pub const OP_BUILD_RUST_NETWORK_JSON_SHADOW: &str = "build-rust-network-json-shadow";
pub const OP_BUILD_RUST_NATIVE_DRY_RUN_PREVIEW: &str = "build-rust-native-dry-run-preview";
pub const OP_BUILD_RUST_SYNC_ENGINE_SHADOW_PREVIEW: &str = "build-rust-sync-engine-shadow-preview";
pub const OP_EVALUATE_RUST_COLLECTOR_AUTHORITY_PILOT: &str =
...
pub const OP_BUILD_COLLECTOR_AUTHORITY_DRY_RUN_BUNDLE: &str =
...
pub const OP_BUILD_COLLECTOR_AUTHORITY_ACTIVATION_PLAN: &str =
    "build-collector-authority-activation-plan";
pub const OP_BUILD_COLLECTOR_AUTHORITY_RUNTIME_CONTRACT: &str =
    "build-collector-authority-runtime-contract";
pub const OP_BUILD_COLLECTOR_AUTHORITY_SWITCH_REHEARSAL: &str =
    "build-collector-authority-switch-rehearsal";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PILOT_EXECUTION_CONTRACT: &str =
    "build-collector-authority-pilot-execution-contract";
pub const OP_EVALUATE_COLLECTOR_AUTHORITY_PILOT_RESULT: &str =
    "evaluate-collector-authority-pilot-result";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PROMOTION_READINESS: &str =
    "build-collector-authority-promotion-readiness";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PROMOTION_EXECUTION_REHEARSAL: &str =
    "build-collector-authority-promotion-execution-rehearsal";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PROMOTION_COMMIT_PLAN: &str =
    "build-collector-authority-promotion-commit-plan";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PROMOTION_CUTOVER_LEDGER: &str =
    "build-collector-authority-promotion-cutover-ledger";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PRODUCTION_FREEZE_GATE: &str =
    "build-collector-authority-production-freeze-gate";
pub const OP_BUILD_COLLECTOR_AUTHORITY_PRODUCTION_SWITCH_CONTRACT: &str =
...
pub const OP_BUILD_RUST_BACKEND_SCHEDULER_HANDOFF_PLAN: &str =
    "build-rust-backend-scheduler-handoff-plan";
pub const OP_BUILD_RUST_RUN_CYCLE_ORCHESTRATOR_HANDOFF_CONTRACT: &str =
    "build-rust-run-cycle-orchestrator-handoff-contract";
pub const OP_BUILD_RUST_CONFIG_STATE_AUTHORITY_HANDOFF_CONTRACT: &str =
    "build-rust-config-state-authority-handoff-contract";
pub const OP_BUILD_RUST_LIVE_COLLECTOR_AUTHORITY_HANDOFF_CONTRACT: &str =
    "build-rust-live-collector-authority-handoff-contract";
pub const OP_BUILD_RUST_CIRCUIT_BUILDER_AUTHORITY_HANDOFF_CONTRACT: &str =
    "build-rust-circuit-builder-authority-handoff-contract";
pub const OP_BUILD_RUST_SYNC_ENGINE_AUTHORITY_HANDOFF_CONTRACT: &str =
    "build-rust-sync-engine-authority-handoff-contract";
pub const OP_BUILD_RUST_APPLY_JOURNAL_ROLLBACK_AUTHORITY_HANDOFF_CONTRACT: &str =
    "build-rust-apply-journal-rollback-authority-handoff-contract";
pub const OP_BUILD_RUST_BACKEND_SERVICE_RUNTIME_HANDOFF_CONTRACT: &str =
    "build-rust-backend-service-runtime-handoff-contract";
pub const OP_BUILD_FULL_RUST_BACKEND_PRODUCTION_READINESS_CONTRACT: &str =
...
pub const OP_BUILD_FULL_RUST_BACKEND_CUTOVER_EXECUTION_CONTRACT: &str =
...
pub const OP_BUILD_RUST_BACKEND_PRODUCTION_ENABLEMENT_CONTRACT: &str =
    "build-rust-backend-production-enablement-contract";
pub const OP_BUILD_PYTHON_BACKEND_REMOVAL_EXECUTION_CONTRACT: &str =
    "build-python-backend-removal-execution-contract";
pub const OP_BUILD_FULL_RUST_BACKEND_REMOVAL_REHEARSAL: &str =
    "build-full-rust-backend-removal-rehearsal";
pub const OP_BUILD_FULL_RUST_BACKEND_PRODUCTION_CUTOVER: &str =
    "build-full-rust-backend-production-cutover";
pub const OP_BUILD_FULL_RUST_BACKEND_PRODUCTION_VERIFIER: &str =
    "build-full-rust-backend-production-verifier";
pub const OP_BUILD_FULL_RUST_BACKEND_POST_RETIREMENT_VERIFIER: &str =
    "build-full-rust-backend-post-retirement-verifier";
pub const OP_BUILD_FULL_RUST_BACKEND_STEADY_STATE_GUARD: &str =
    "build-full-rust-backend-steady-state-guard";
pub const OP_BUILD_FULL_RUST_BACKEND_PRODUCTION_DRIFT_MONITOR: &str =
    "build-full-rust-backend-production-drift-monitor";
pub const OP_BUILD_FULL_RUST_BACKEND_PRODUCTION_AUDIT_SENTINEL: &str =
    "build-full-rust-backend-production-audit-sentinel";
pub const OP_BUILD_PYTHON_LEGACY_RETIREMENT_INVENTORY: &str =
...
pub const OP_RUN_RUST_CYCLE_AUTHORITY: &str = "run-rust-cycle-authority";
...
pub fn advertised_operations() -> &'static [&'static str] {
    &[
        ...
        OP_BUILD_RUST_NETWORK_JSON_SHADOW,
        OP_BUILD_RUST_NATIVE_DRY_RUN_PREVIEW,
        OP_BUILD_RUST_SYNC_ENGINE_SHADOW_PREVIEW,
...
pub fn self_test_payload(payload: &Value) -> (Value, Vec<Diagnostic>, Vec<Diagnostic>) {
    ...
    let has_scheduler =
        operations.contains(&OP_SCHEDULER_STATUS) && operations.contains(&OP_SCHEDULER_RUN_ONCE);
    checks.push(check(
    ...
    let has_run_cycle_authority = operations.contains(&OP_RUN_RUST_CYCLE_AUTHORITY);
    checks.push(check(
        "rust_run_cycle_authority_advertised",
        has_run_cycle_authority,
        json!({"operation": OP_RUN_RUST_CYCLE_AUTHORITY}),
    ));
    if !has_run_cycle_authority {
        errors.push(Diagnostic::error(
            "run_cycle_authority_operation_not_advertised",
            Some("operations".to_string()),
            "run-rust-cycle-authority is not advertised by the Rust core.",
        ));
    }
    ...
    checks.push(check(
        "bandwidth_unit_parser",
        unit_ok,
        json!({"input":"1G", "mbps": unit_mbps}),
    ));
    if !unit_ok {
        errors.push(Diagnostic::error(
            "self_test_bandwidth_unit_failed",
            Some("bandwidth".to_string()),
            "1G should parse as 1000 Mbps.",
        ));
    }

    let rate = parse_rate_limit("10M/5M");
    let rate_ok = (rate.download_mbps - 10.0).abs() < f64::EPSILON
        && (rate.upload_mbps - 5.0).abs() < f64::EPSILON;
    checks.push(check(
        "bandwidth_rate_limit_parser",
        rate_ok,
        json!({"download_mbps": rate.download_mbps, "upload_mbps": rate.upload_mbps}),
    ));
    if !rate_ok {
        errors.push(Diagnostic::error(
            "self_test_rate_limit_failed",
            Some("bandwidth".to_string()),
            "10M/5M should parse as 10/5 Mbps.",
        ));
    }

    let comment_ok = parse_comment_bandwidth("PLAN|15M/15M")
        .map(|v| {
            (v.download_mbps - 15.0).abs() < f64::EPSILON
                && (v.upload_mbps - 15.0).abs() < f64::EPSILON
        })
        .unwrap_or(false);
    checks.push(check(
        "bandwidth_comment_parser",
        comment_ok,
        json!({"input":"PLAN|15M/15M"}),
    ));
    if !comment_ok {
        errors.push(Diagnostic::error(
            "self_test_comment_bandwidth_failed",
            Some("bandwidth".to_string()),
            "PLAN|15M/15M should parse as 15/15 Mbps.",
        ));
    }
    ...
    let (routeros_plan, routeros_plan_errors, _routeros_plan_warnings) =
        build_routeros_collector_plan_payload(&routeros_plan_payload);
    ...
    let (routeros_results, routeros_results_errors, _routeros_results_warnings) =
        validate_routeros_read_results_payload(&routeros_results_payload);
    ...
    let (transport_result, transport_errors, _transport_warnings) =
    ...
    let transport_ok = transport_errors.is_empty()
        && transport_result
            ...
            .unwrap_or(1)
            == 0
        && transport_result.get("status").and_then(Value::as_str)
            == Some("ready_for_future_transport");
    ...
    let (live_pilot, live_pilot_errors, _live_pilot_warnings) =
        build_routeros_live_read_pilot_payload(&live_pilot_payload);
    ...
    let (read_pilot, read_pilot_errors, _read_pilot_warnings) =
        run_routeros_read_pilot_payload(&read_pilot_payload);
    ...
    let (api_sentence, api_sentence_errors, _api_sentence_warnings) =
        build_routeros_api_sentence_payload(&api_sentence_payload);
    ...
    let (api_reply, api_reply_errors, _api_reply_warnings) =
        decode_routeros_api_reply_payload(&api_reply_payload);
    ...
    let (api_frame, api_frame_errors, _api_frame_warnings) =
        codec_routeros_api_frame_payload(&api_frame_payload);
    ...
    let (offline_session, offline_session_errors, _offline_session_warnings) =
        run_routeros_offline_session_payload(&offline_session_payload);
    let offline_session_ok = offline_session_errors.is_empty()
        && offline_session.get("status").and_then(Value::as_str)
            == Some("offline_session_complete")
        && offline_session
            .get("connection_attempt_count")
            .and_then(Value::as_u64)
            .unwrap_or(1)
    ...
    let (tcp_probe, tcp_probe_errors, _tcp_probe_warnings) =
        run_routeros_tcp_connectivity_pilot_payload(&json!({
    ...
    let tcp_probe_ok = tcp_probe_errors.is_empty()
        ...
        && tcp_probe
            ...
            == 0;
    checks.push(check(
        "routeros_tcp_connectivity_rehearsal",
        tcp_probe_ok,
        json!({
    ...
    let (auth_plan, auth_plan_errors, _auth_plan_warnings) = build_routeros_auth_plan_payload(
        &json!({
    ...
    let (auth_handshake, auth_handshake_errors, _auth_handshake_warnings) =
        run_routeros_auth_handshake_payload(&json!({
            "router": {"name":"selftest", "address":"127.0.0.1", "port":8728, "username":"admin", "password":"selftest_secret"},
            "adapter": "fixture",
            "execute": true,
    ...
    let auth_handshake_ok = auth_handshake_errors.is_empty()
        ...
        && auth_handshake
            ...
            == 0;
    checks.push(check(
        "routeros_auth_handshake_fixture",
        auth_handshake_ok,
        json!({
    ...
    let (auth_session, auth_session_errors, _auth_session_warnings) =
        build_routeros_auth_session_contract_payload(&auth_session_payload);
    ...
    let (authenticated_read, authenticated_read_errors, _authenticated_read_warnings) =
        run_routeros_authenticated_read_fixture_payload(&authenticated_read_payload);
    let authenticated_read_ok = authenticated_read_errors.is_empty()
        && authenticated_read.get("status").and_then(Value::as_str)
            == Some("authenticated_read_fixture_complete")
        && authenticated_read
            .get("connection_attempt_count")
            .and_then(Value::as_u64)
            .unwrap_or(1)
        ...
        && authenticated_read
            ...
            == 1;
    checks.push(check(
        "routeros_authenticated_read_fixture",
    ...
    if !authenticated_read_ok {
        errors.push(Diagnostic::error(
            "self_test_routeros_authenticated_read_failed",
            Some("run-routeros-authenticated-read-fixture".to_string()),
            "Self-test authenticated read fixture should complete without network access.",
        ));
    }
    ...
    let (live_read_adapter, live_read_adapter_errors, _live_read_adapter_warnings) =
        run_routeros_live_read_adapter_pilot_payload(&live_read_adapter_payload);
    let live_read_adapter_ok = live_read_adapter_errors.is_empty()
        && live_read_adapter.get("status").and_then(Value::as_str)
        ...
        && live_read_adapter
            ...
            == 0;
    checks.push(check(
        "routeros_live_read_adapter_contract",
        live_read_adapter_ok,
        json!({
    ...
    let (collector_authority, collector_authority_errors, _collector_authority_warnings) =
        evaluate_rust_collector_authority_pilot_payload(&collector_authority_payload);
    let collector_authority_ok = collector_authority_errors.is_empty()
        && collector_authority.get("status").and_then(Value::as_str)
            == Some("collector_authority_pilot_gate_ready")
        && collector_authority
            .get("gates_ready")
            .and_then(Value::as_bool)
            .unwrap_or(false)
        && collector_authority
            .get("full_rust_backend")
            .and_then(Value::as_bool)
            .unwrap_or(true)
            == false;
    checks.push(check(
        "rust_collector_authority_pilot_gate",
        collector_authority_ok,
        json!({
    ...
    let (collector_manifest, collector_manifest_errors, _collector_manifest_warnings) =
        build_collector_authority_manifest_payload(&collector_manifest_payload);
    let collector_manifest_ok = collector_manifest_errors.is_empty()
        && collector_manifest.get("status").and_then(Value::as_str)
            == Some("collector_authority_manifest_ready")
        && collector_manifest
            .get("ready_count")
            .and_then(Value::as_u64)
            .unwrap_or(0)
        ...
        && collector_manifest
            ...
            == false;
    checks.push(check(
        "collector_authority_decision_manifest",
        collector_manifest_ok,
        json!({
    ...
    let (
        collector_authority_selection,
        collector_authority_selection_errors,
        _collector_authority_selection_warnings,
    ) = build_collector_authority_selection_payload(&collector_authority_selection_payload);
    let collector_authority_selection_ok = collector_authority_selection_errors.is_empty()
        && collector_authority_selection
            .get("status")
            .and_then(Value::as_str)
            == Some("collector_authority_dry_run_selection_ready")
        && collector_authority_selection
            .get("rust_shadow_count")
            .and_then(Value::as_u64)
            .unwrap_or(0)
            >= 1
        && collector_authority_selection
            ...
            == false;
    checks.push(check(
        "collector_authority_dry_run_selection",
        collector_authority_selection_ok,
        json!({
    ...
    let (
        collector_authority_dry_run,
        collector_authority_dry_run_errors,
        _collector_authority_dry_run_warnings,
    ) = build_collector_authority_dry_run_bundle_payload(&collector_authority_dry_run_payload);
    let collector_authority_dry_run_ok = collector_authority_dry_run_errors.is_empty()
        && collector_authority_dry_run
            .get("status")
            .and_then(Value::as_str)
            == Some("collector_authority_dry_run_bundle_ready")
        && collector_authority_dry_run
            ...
            == 1
        && collector_authority_dry_run
            .get("collector_output_can_drive_cleanup")
            .and_then(Value::as_bool)
            .unwrap_or(true)
            == false;
    checks.push(check(
        "collector_authority_dry_run_bundle",
        collector_authority_dry_run_ok,
        json!({
    ...
    if let Some(obj) = run_cycle_shadow_payload.as_object_mut() {
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "run_cycle_rust_shadow_report_enabled".to_string(),
                json!(true),
            );
            rc.insert(
                "run_cycle_rust_shadow_report_pilot".to_string(),
                json!(true),
            );
        }
    }
    let (run_cycle_shadow, run_cycle_shadow_errors, _run_cycle_shadow_warnings) =
        build_run_cycle_rust_shadow_report_payload(&run_cycle_shadow_payload);
    let run_cycle_shadow_ok = run_cycle_shadow_errors.is_empty()
        && run_cycle_shadow.get("status").and_then(Value::as_str)
    ...
    if let Some(obj) = activation_payload.as_object_mut() {
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_activation_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_activation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_activation_mode".to_string(),
                json!("rust_collector_authority_pilot"),
            );
            rc.insert(
                "collector_authority_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_min_shadow_cycles".to_string(),
                json!(3),
            );
        }
    }
    let (activation_plan, activation_errors, _activation_warnings) =
        build_collector_authority_activation_plan_payload(&activation_payload);
    let activation_ok = activation_errors.is_empty()
        && activation_plan.get("status").and_then(Value::as_str)
            == Some("collector_authority_activation_ready_for_pilot")
        && activation_plan
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false);
    checks.push(check(
    ...
    if let Some(obj) = runtime_payload.as_object_mut() {
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            ...
            rc.insert(
                "allow_collector_authority_runtime_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_runtime_mode".to_string(),
                json!("rust_collector_authority_runtime_contract"),
            );
            rc.insert(
                "collector_authority_runtime_require_activation_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_runtime_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_runtime_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (runtime_contract, runtime_errors, _runtime_warnings) =
        build_collector_authority_runtime_contract_payload(&runtime_payload);
    let runtime_ok = runtime_errors.is_empty()
        && runtime_contract.get("status").and_then(Value::as_str)
            == Some("collector_authority_runtime_contract_ready")
        && runtime_contract
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = switch_payload.as_object_mut() {
        obj.insert(
            ...
            json!("CONFIRM_COLLECTOR_AUTHORITY_REHEARSAL"),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_switch_rehearsal_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_switch_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_switch_mode".to_string(),
                json!("rust_collector_authority_switch_rehearsal"),
            );
            rc.insert(
                "collector_authority_switch_require_runtime_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_switch_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_switch_require_manual_confirmation".to_string(),
                json!(true),
            );
        }
    }
    let (switch_rehearsal, switch_errors, _switch_warnings) =
        build_collector_authority_switch_rehearsal_payload(&switch_payload);
    let switch_ok = switch_errors.is_empty()
        && switch_rehearsal.get("status").and_then(Value::as_str)
            == Some("collector_authority_switch_rehearsal_ready")
        && switch_rehearsal
            .get("collector_authority_switch_executed")
            .and_then(Value::as_bool)
            == Some(false)
        && switch_rehearsal
            .get("production_collector_authority_switched")
    ...
    if !switch_ok {
        errors.push(Diagnostic::error(
            "self_test_collector_authority_switch_failed",
            Some("build-collector-authority-switch-rehearsal".to_string()),
            "Self-test collector authority switch rehearsal should be ready but non-mutating.",
        ));
    }
    ...
    if let Some(obj) = pilot_execution_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PILOT_EXECUTION"),
        );
        obj.insert(
            "collector_authority_switch_confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_REHEARSAL"),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_pilot_execution_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_pilot_execution_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_execution_mode".to_string(),
                json!("rust_collector_authority_pilot_execution_contract"),
            );
            rc.insert(
                "collector_authority_pilot_execution_require_switch_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_execution_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_execution_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_execution_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (pilot_execution, pilot_execution_errors, _pilot_execution_warnings) =
        build_collector_authority_pilot_execution_contract_payload(&pilot_execution_payload);
    let pilot_execution_ok = pilot_execution_errors.is_empty()
        && pilot_execution.get("status").and_then(Value::as_str)
            == Some("collector_authority_pilot_execution_contract_ready")
        && pilot_execution
            .get("collector_authority_pilot_execution_executed")
            .and_then(Value::as_bool)
            == Some(false)
        && pilot_execution
            .get("production_collector_authority_switched")
    ...
    if let Some(obj) = pilot_result_payload.as_object_mut() {
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_pilot_result_evaluator_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_pilot_result_evaluation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_result_mode".to_string(),
                json!("rust_collector_authority_pilot_result_evaluation"),
            );
            rc.insert(
                "collector_authority_pilot_result_require_execution_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_result_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_result_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_result_require_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_pilot_result_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (pilot_result, pilot_result_errors, _pilot_result_warnings) =
        evaluate_collector_authority_pilot_result_payload(&pilot_result_payload);
    let pilot_result_ok = pilot_result_errors.is_empty()
        && pilot_result.get("status").and_then(Value::as_str)
            == Some("collector_authority_pilot_result_pass")
        && pilot_result
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = promotion_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PROMOTION_READINESS"),
        );
        obj.insert(
            "collector_authority_pilot_result_evaluation".to_string(),
            json!(pilot_result.clone()),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_promotion_readiness_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_promotion_readiness".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_readiness_mode".to_string(),
                json!("rust_collector_authority_promotion_readiness"),
            );
            rc.insert(
                "collector_authority_promotion_require_pilot_result".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (promotion_readiness, promotion_errors, _promotion_warnings) =
        build_collector_authority_promotion_readiness_payload(&promotion_payload);
    let promotion_ok = promotion_errors.is_empty()
        && promotion_readiness.get("status").and_then(Value::as_str)
            == Some("collector_authority_promotion_readiness_ready")
        && promotion_readiness
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = promotion_execution_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PROMOTION_EXECUTION_REHEARSAL"),
        );
        obj.insert(
            "collector_authority_promotion_readiness".to_string(),
            json!(promotion_readiness.clone()),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_promotion_execution_rehearsal_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_promotion_execution_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_execution_mode".to_string(),
                json!("rehearsal_only"),
            );
            rc.insert(
                "collector_authority_promotion_execution_require_readiness".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_execution_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_execution_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_execution_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_execution_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (promotion_execution, promotion_execution_errors, _promotion_execution_warnings) =
        build_collector_authority_promotion_execution_rehearsal_payload(
            &promotion_execution_payload,
        );
    let promotion_execution_ok = promotion_execution_errors.is_empty()
        && promotion_execution.get("status").and_then(Value::as_str)
            == Some("collector_authority_promotion_execution_rehearsal_ready")
        && promotion_execution
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = promotion_commit_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PROMOTION_COMMIT_PLAN"),
        );
        obj.insert(
            "collector_authority_promotion_execution_rehearsal".to_string(),
            json!(promotion_execution.clone()),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_promotion_commit_plan_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_promotion_commit_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_commit_mode".to_string(),
                json!("plan_only"),
            );
            rc.insert(
                "collector_authority_promotion_commit_require_execution_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_commit_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_commit_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_commit_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_commit_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (promotion_commit, promotion_commit_errors, _promotion_commit_warnings) =
        build_collector_authority_promotion_commit_plan_payload(&promotion_commit_payload);
    let promotion_commit_ok = promotion_commit_errors.is_empty()
        && promotion_commit.get("status").and_then(Value::as_str)
            == Some("collector_authority_promotion_commit_plan_ready")
        && promotion_commit
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = promotion_cutover_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PROMOTION_CUTOVER_LEDGER"),
        );
        obj.insert(
            "collector_authority_promotion_commit_confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PROMOTION_COMMIT_PLAN"),
        );
        obj.insert("rollback_path".to_string(), json!("python_fallback_revert"));
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_promotion_cutover_ledger_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_promotion_cutover_ledger".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_mode".to_string(),
                json!("ledger_only"),
            );
            rc.insert(
                "collector_authority_promotion_cutover_require_commit_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_promotion_cutover_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (promotion_cutover, promotion_cutover_errors, _promotion_cutover_warnings) =
        build_collector_authority_promotion_cutover_ledger_payload(&promotion_cutover_payload);
    let promotion_cutover_ok = promotion_cutover_errors.is_empty()
        && promotion_cutover.get("status").and_then(Value::as_str)
            == Some("collector_authority_promotion_cutover_ledger_ready")
        && promotion_cutover
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = production_freeze_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PRODUCTION_FREEZE_GATE"),
        );
        obj.insert(
            "collector_authority_promotion_cutover_ledger".to_string(),
            json!(promotion_cutover.clone()),
        );
        obj.insert(
            "maintenance_window".to_string(),
            json!("2026-05-20T23:00:00+08:00/PT30M"),
        );
        obj.insert("operator_acknowledged".to_string(), json!(true));
        obj.insert("rollback_path".to_string(), json!("python_fallback_revert"));
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_production_freeze_gate_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_production_freeze_gate".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_mode".to_string(),
                json!("freeze_only"),
            );
            rc.insert(
                "collector_authority_production_freeze_require_cutover_ledger".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_maintenance_window".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_freeze_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (production_freeze, production_freeze_errors, _production_freeze_warnings) =
        build_collector_authority_production_freeze_gate_payload(&production_freeze_payload);
    let production_freeze_ok = production_freeze_errors.is_empty()
        && production_freeze.get("status").and_then(Value::as_str)
            == Some("collector_authority_production_freeze_gate_ready")
        && production_freeze
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
        && production_freeze
            .get("collector_authority_production_switch_executed")
            .and_then(Value::as_bool)
            == Some(false)
        && production_freeze
            .get("python_backend_removable")
            .and_then(Value::as_bool)
            == Some(false);
    checks.push(check(
        "collector_authority_production_freeze_gate",
        production_freeze_ok,
        json!({
    ...
    if let Some(obj) = production_switch_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_COLLECTOR_AUTHORITY_PRODUCTION_SWITCH_CONTRACT"),
        );
        obj.insert(
            "collector_authority_production_freeze_gate".to_string(),
            json!(production_freeze.clone()),
        );
        obj.insert(
            "maintenance_window".to_string(),
            json!("2026-05-20T23:00:00+08:00/PT30M"),
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "collector_authority_production_switch_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_collector_authority_production_switch_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "collector_authority_production_switch_require_freeze_gate".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_no_cleanup_apply".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_maintenance_window".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "collector_authority_production_switch_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (production_switch, production_switch_errors, _production_switch_warnings) =
        build_collector_authority_production_switch_contract_payload(&production_switch_payload);
    let production_switch_ok = production_switch_errors.is_empty()
        && production_switch.get("status").and_then(Value::as_str)
            == Some("collector_authority_production_switch_contract_ready")
        && production_switch
            .get("production_collector_authority_switched")
            .and_then(Value::as_bool)
            == Some(false)
    ...
    if let Some(obj) = api_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_BACKEND_API_HANDOFF_PLAN"),
        );
        obj.insert(
            "collector_authority_production_switch_contract".to_string(),
            json!(production_switch.clone()),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_backend_api_handoff_plan_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_backend_api_handoff_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_mode".to_string(),
                json!("plan_only"),
            );
            rc.insert(
                "rust_backend_api_handoff_require_production_switch_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_require_python_backend_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_require_webui_compatibility".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_require_route_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_api_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (api_handoff, api_handoff_errors, _api_handoff_warnings) =
        build_rust_backend_api_handoff_plan_payload(&api_handoff_payload);
    let api_handoff_ok = api_handoff_errors.is_empty()
        && api_handoff.get("status").and_then(Value::as_str)
            == Some("rust_backend_api_handoff_plan_ready")
        && api_handoff
            .get("rust_backend_api_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && api_handoff
            .get("python_backend_removed")
            .and_then(Value::as_bool)
            == Some(false)
        && api_handoff
            .get("webui_ux_unchanged")
            .and_then(Value::as_bool)
            == Some(true);
    checks.push(check(
    ...
    if let Some(obj) = scheduler_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_BACKEND_SCHEDULER_RUN_CYCLE_HANDOFF_PLAN"),
        );
        obj.insert(
            "rust_backend_api_handoff_plan".to_string(),
            json!(api_handoff.clone()),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_backend_scheduler_handoff_plan_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_backend_scheduler_handoff_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_mode".to_string(),
                json!("plan_only"),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_api_handoff".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_run_cycle_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_scheduler_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_scheduler_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (scheduler_handoff, scheduler_handoff_errors, _scheduler_handoff_warnings) =
        build_rust_backend_scheduler_handoff_plan_payload(&scheduler_handoff_payload);
    let scheduler_handoff_ok = scheduler_handoff_errors.is_empty()
        && scheduler_handoff.get("status").and_then(Value::as_str)
            == Some("rust_backend_scheduler_handoff_plan_ready")
        && scheduler_handoff
            .get("rust_backend_scheduler_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && scheduler_handoff
            .get("rust_scheduler_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && scheduler_handoff
            .get("rust_run_cycle_authoritative")
            .and_then(Value::as_bool)
            == Some(false);
    ...
    if let Some(obj) = run_cycle_orchestrator_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_RUN_CYCLE_ORCHESTRATOR_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_backend_scheduler_handoff_plan".to_string(),
            json!(scheduler_handoff.clone()),
        );
        obj.insert(
            "run_cycle_orchestrator_manifest_ready".to_string(),
            json!(true),
        );
        obj.insert("run_cycle_shadow_ready".to_string(), json!(true));
        obj.insert("run_cycle_shadow_count".to_string(), json!(3));
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_run_cycle_orchestrator_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_scheduler_handoff".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_run_cycle_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_config_state_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_run_cycle_orchestrator_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (run_cycle_orchestrator, run_cycle_orchestrator_errors, _run_cycle_orchestrator_warnings) =
        build_rust_run_cycle_orchestrator_handoff_contract_payload(&run_cycle_orchestrator_payload);
    let run_cycle_orchestrator_ok = run_cycle_orchestrator_errors.is_empty()
        && run_cycle_orchestrator.get("status").and_then(Value::as_str)
            == Some("rust_run_cycle_orchestrator_handoff_contract_ready")
        && run_cycle_orchestrator
            .get("rust_run_cycle_orchestrator_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && run_cycle_orchestrator
            .get("rust_run_cycle_authoritative")
    ...
    if let Some(obj) = config_state_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_CONFIG_STATE_AUTHORITY_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_run_cycle_orchestrator_handoff_contract".to_string(),
            json!(run_cycle_orchestrator.clone()),
        );
        obj.insert("config_state_shadow_ready".to_string(), json!(true));
        obj.insert("config_state_shadow_count".to_string(), json!(3));
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_config_state_authority_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_config_state_authority_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_run_cycle_orchestrator".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_config_state_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_atomic_writer_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_transaction_journal_shadow"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_audit_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_rollback_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_config_state_authority_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (config_state_handoff, config_state_handoff_errors, _config_state_handoff_warnings) =
        build_rust_config_state_authority_handoff_contract_payload(&config_state_handoff_payload);
    let config_state_handoff_ok = config_state_handoff_errors.is_empty()
        && config_state_handoff.get("status").and_then(Value::as_str)
            == Some("rust_config_state_authority_handoff_contract_ready")
        && config_state_handoff
            .get("rust_config_state_authority_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && config_state_handoff
            .get("rust_config_state_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && config_state_handoff
            .get("python_config_state_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = live_collector_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_LIVE_COLLECTOR_AUTHORITY_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_config_state_authority_handoff_contract".to_string(),
            json!(config_state_handoff.clone()),
        );
        obj.insert("live_collector_shadow_ready".to_string(), json!(true));
        obj.insert("live_collector_shadow_count".to_string(), json!(3));
        obj.insert(
            "routeros_live_adapter_shadow_ready".to_string(),
            json!(true),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_live_collector_authority_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_live_collector_authority_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_config_state_authority".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_live_collector_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_routeros_adapter_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_collector_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_live_collector_authority_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (live_collector_handoff, live_collector_handoff_errors, _live_collector_handoff_warnings) =
        build_rust_live_collector_authority_handoff_contract_payload(
            &live_collector_handoff_payload,
        );
    let live_collector_handoff_ok = live_collector_handoff_errors.is_empty()
        && live_collector_handoff.get("status").and_then(Value::as_str)
            == Some("rust_live_collector_authority_handoff_contract_ready")
        && live_collector_handoff
            .get("rust_live_collector_authority_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && live_collector_handoff
            .get("rust_live_collector_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && live_collector_handoff
            .get("python_live_collector_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = circuit_builder_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_CIRCUIT_BUILDER_AUTHORITY_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_live_collector_authority_handoff_contract".to_string(),
            json!(live_collector_handoff.clone()),
        );
        obj.insert("circuit_builder_shadow_ready".to_string(), json!(true));
        obj.insert("circuit_builder_shadow_count".to_string(), json!(3));
        obj.insert(
            "shaped_devices_render_parity_ready".to_string(),
            json!(true),
        );
        obj.insert(
            "shaped_devices_render_parity_score".to_string(),
            json!(100.0),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_circuit_builder_authority_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_circuit_builder_authority_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_live_collector_authority"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_circuit_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_shaped_devices_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_parent_integrity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_circuit_builder_authority_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        circuit_builder_handoff,
        circuit_builder_handoff_errors,
        _circuit_builder_handoff_warnings,
    ) = build_rust_circuit_builder_authority_handoff_contract_payload(
        &circuit_builder_handoff_payload,
    );
    let circuit_builder_handoff_ok = circuit_builder_handoff_errors.is_empty()
        && circuit_builder_handoff
            .get("status")
            .and_then(Value::as_str)
            == Some("rust_circuit_builder_authority_handoff_contract_ready")
        && circuit_builder_handoff
            .get("rust_circuit_builder_authority_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && circuit_builder_handoff
            .get("rust_circuit_builder_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && circuit_builder_handoff
            .get("python_circuit_builder_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = sync_engine_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_SYNC_ENGINE_AUTHORITY_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_circuit_builder_authority_handoff_contract".to_string(),
            json!(circuit_builder_handoff.clone()),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_sync_engine_authority_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_sync_engine_authority_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_circuit_builder_authority".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_sync_plan_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_diff_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_apply_manifest_preview".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_cleanup_safety".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_sync_engine_authority_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (sync_engine_handoff, sync_engine_handoff_errors, _sync_engine_handoff_warnings) =
        build_rust_sync_engine_authority_handoff_contract_payload(&sync_engine_handoff_payload);
    let sync_engine_handoff_ok = sync_engine_handoff_errors.is_empty()
        && sync_engine_handoff.get("status").and_then(Value::as_str)
            == Some("rust_sync_engine_authority_handoff_contract_ready")
        && sync_engine_handoff
            .get("rust_sync_engine_authority_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && sync_engine_handoff
            .get("rust_sync_engine_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && sync_engine_handoff
            .get("python_sync_engine_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = apply_journal_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_APPLY_JOURNAL_ROLLBACK_AUTHORITY_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_sync_engine_authority_handoff_contract".to_string(),
            json!(sync_engine_handoff.clone()),
        );
        obj.insert("apply_transaction_shadow_ready".to_string(), json!(true));
        obj.insert("apply_manifest_replay_ready".to_string(), json!(true));
        obj.insert(
            "apply_transaction_shadow_blocker_count".to_string(),
            json!(0),
        );
        obj.insert("transaction_journal_shadow_ready".to_string(), json!(true));
        obj.insert("journal_replay_parity_ready".to_string(), json!(true));
        obj.insert(
            "transaction_journal_shadow_error_count".to_string(),
            json!(0),
        );
        obj.insert("rollback_manifest_shadow_ready".to_string(), json!(true));
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_apply_journal_rollback_authority_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_sync_engine_authority"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_manual_confirmation"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_apply_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_journal_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_rollback_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_audit_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_apply_journal_rollback_authority_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (apply_journal_handoff, apply_journal_handoff_errors, _apply_journal_handoff_warnings) =
        build_rust_apply_journal_rollback_authority_handoff_contract_payload(
            &apply_journal_handoff_payload,
        );
    let apply_journal_handoff_ok = apply_journal_handoff_errors.is_empty()
        && apply_journal_handoff.get("status").and_then(Value::as_str)
            == Some("rust_apply_journal_rollback_authority_handoff_contract_ready")
        && apply_journal_handoff
            .get("rust_apply_journal_rollback_authority_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && apply_journal_handoff
            .get("rust_apply_journal_rollback_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && apply_journal_handoff
            .get("python_apply_journal_rollback_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = service_runtime_handoff_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_BACKEND_SERVICE_RUNTIME_HANDOFF_CONTRACT"),
        );
        obj.insert(
            "rust_apply_journal_rollback_authority_handoff_contract".to_string(),
            json!(apply_journal_handoff.clone()),
        );
        ...
        obj.insert(
            "webui_static_asset_paths_unchanged".to_string(),
            json!(true),
        );
        obj.insert("static_asset_compat_error_count".to_string(), json!(0));
        obj.insert(
            "rust_service_supervision_shadow_ready".to_string(),
            json!(true),
        );
        obj.insert("rust_daemon_socket_shadow_ready".to_string(), json!(true));
        obj.insert(
            "rust_service_healthcheck_shadow_ready".to_string(),
            json!(true),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_backend_service_runtime_handoff_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_backend_service_runtime_handoff_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_apply_journal_rollback_authority"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_route_parity".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_static_assets".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_service_supervision".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_api_shadow".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_service_runtime_handoff_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        service_runtime_handoff,
        service_runtime_handoff_errors,
        _service_runtime_handoff_warnings,
    ) = build_rust_backend_service_runtime_handoff_contract_payload(
        &service_runtime_handoff_payload,
    );
    let service_runtime_handoff_ok = service_runtime_handoff_errors.is_empty()
        && service_runtime_handoff
            .get("status")
            .and_then(Value::as_str)
            == Some("rust_backend_service_runtime_handoff_contract_ready")
        && service_runtime_handoff
            .get("rust_backend_service_runtime_handoff_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && service_runtime_handoff
            .get("rust_service_runtime_authoritative")
            .and_then(Value::as_bool)
            == Some(false)
        && service_runtime_handoff
            .get("python_service_runtime_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    ...
    if let Some(obj) = full_backend_readiness_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_PRODUCTION_READINESS_CONTRACT"),
        );
        obj.insert(
            "rust_backend_service_runtime_handoff_contract".to_string(),
            json!(service_runtime_handoff.clone()),
        );
        obj.insert("webui_ux_unchanged".to_string(), json!(true));
        obj.insert(
            "webui_static_asset_paths_unchanged".to_string(),
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_production_readiness_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_production_readiness_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_service_runtime".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_operator_final_review".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_readiness_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (full_backend_readiness, full_backend_readiness_errors, _full_backend_readiness_warnings) =
        build_full_rust_backend_production_readiness_contract_payload(
            &full_backend_readiness_payload,
        );
    let full_backend_readiness_ok = full_backend_readiness_errors.is_empty()
        && full_backend_readiness.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_production_readiness_contract_ready")
        && full_backend_readiness
            .get("full_rust_backend_production_readiness_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && full_backend_readiness
            .get("full_rust_backend_production_enabled")
            .and_then(Value::as_bool)
            == Some(false)
        && full_backend_readiness
            .get("python_backend_removed")
    ...
    if let Some(obj) = full_backend_cutover_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_CUTOVER_PLAN"),
        );
        obj.insert(
            "full_rust_backend_production_readiness_contract".to_string(),
            json!(full_backend_readiness.clone()),
        );
        obj.insert(
            "rollback_path".to_string(),
            json!("python_backend_reenable_and_flask_route_restore"),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_cutover_plan_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_cutover_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_mode".to_string(),
                json!("plan_only"),
            );
            rc.insert(
                "full_rust_backend_cutover_require_production_readiness".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_operator_approval".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (full_backend_cutover, full_backend_cutover_errors, _full_backend_cutover_warnings) =
        build_full_rust_backend_cutover_plan_payload(&full_backend_cutover_payload);
    let full_backend_cutover_ok = full_backend_cutover_errors.is_empty()
        && full_backend_cutover.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_cutover_plan_ready")
        && full_backend_cutover
            .get("full_rust_backend_cutover_plan_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && full_backend_cutover
            .get("python_backend_removed")
    ...
    if let Some(obj) = full_backend_cutover_execution_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_CUTOVER_EXECUTION_CONTRACT"),
        );
        obj.insert(
            "full_rust_backend_cutover_plan".to_string(),
            json!(full_backend_cutover.clone()),
        );
        obj.insert("operator_cutover_execution_ack".to_string(), json!(true));
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_cutover_execution_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_cutover_execution_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_cutover_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_cutover_execution_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        full_backend_cutover_execution,
        full_backend_cutover_execution_errors,
        _full_backend_cutover_execution_warnings,
    ) = build_full_rust_backend_cutover_execution_contract_payload(
        &full_backend_cutover_execution_payload,
    );
    let full_backend_cutover_execution_ok = full_backend_cutover_execution_errors.is_empty()
        && full_backend_cutover_execution
            .get("status")
            .and_then(Value::as_str)
            == Some("full_rust_backend_cutover_execution_contract_ready")
        && full_backend_cutover_execution
            .get("full_rust_backend_cutover_execution_contract_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && full_backend_cutover_execution
            .get("python_backend_removed")
            ...
            == Some(false)
        && full_backend_cutover_execution
            .get("api_traffic_switched_to_rust")
            .and_then(Value::as_bool)
            == Some(false);
    ...
    if let Some(obj) = python_retirement_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_PYTHON_BACKEND_RETIREMENT_PLAN"),
        );
        obj.insert(
            "full_rust_backend_cutover_execution_contract".to_string(),
            json!(full_backend_cutover_execution.clone()),
        );
        obj.insert("operator_python_retirement_ack".to_string(), json!(true));
        obj.insert(
            "rollback_path".to_string(),
            json!("restore_python_backend_and_flask_routes"),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "python_backend_retirement_plan_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_python_backend_retirement_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_mode".to_string(),
                json!("plan_only"),
            );
            rc.insert(
                "python_backend_retirement_require_cutover_execution_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_retirement_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (python_retirement, python_retirement_errors, _python_retirement_warnings) =
        build_python_backend_retirement_plan_payload(&python_retirement_payload);
    let python_retirement_ok = python_retirement_errors.is_empty()
        && python_retirement.get("status").and_then(Value::as_str)
            == Some("python_backend_retirement_plan_ready")
        && python_retirement
            .get("python_backend_retirement_plan_ready")
            .and_then(Value::as_bool)
            == Some(true)
        && python_retirement
            .get("python_backend_removed")
    ...
    if let Some(obj) = rust_backend_enablement_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_RUST_BACKEND_PRODUCTION_ENABLEMENT_CONTRACT"),
        );
        obj.insert(
            "python_backend_retirement_plan".to_string(),
            json!(python_retirement.clone()),
        );
        obj.insert(
            "operator_rust_backend_enablement_ack".to_string(),
            json!(true),
        );
        obj.insert(
            "rollback_path".to_string(),
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "rust_backend_production_enablement_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_rust_backend_production_enablement_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "rust_backend_production_enablement_require_python_retirement_plan".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "rust_backend_production_enablement_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        rust_backend_enablement,
        rust_backend_enablement_errors,
        _rust_backend_enablement_warnings,
    ) = build_rust_backend_production_enablement_contract_payload(&rust_backend_enablement_payload);
    let rust_backend_enablement_ok = rust_backend_enablement_errors.is_empty()
        && rust_backend_enablement
            .get("status")
            .and_then(Value::as_str)
            == Some("rust_backend_production_enablement_contract_ready")
        && rust_backend_enablement
            .get("full_rust_backend_candidate")
            .and_then(Value::as_bool)
            == Some(true)
        && rust_backend_enablement
            .get("python_backend_removed")
    ...
    if let Some(obj) = python_removal_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_PYTHON_BACKEND_REMOVAL_EXECUTION_CONTRACT"),
        );
        obj.insert(
            "rust_backend_production_enablement_contract".to_string(),
            json!(rust_backend_enablement.clone()),
        );
        obj.insert(
            "operator_python_backend_removal_execution_ack".to_string(),
            json!(true),
        );
        obj.insert(
            "rollback_path".to_string(),
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "python_backend_removal_execution_contract_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_python_backend_removal_execution_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_mode".to_string(),
                json!("contract_only"),
            );
            rc.insert(
                "python_backend_removal_execution_require_rust_enablement_contract".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "python_backend_removal_execution_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (python_removal, python_removal_errors, _python_removal_warnings) =
        build_python_backend_removal_execution_contract_payload(&python_removal_payload);
    let python_removal_ok = python_removal_errors.is_empty()
        && python_removal.get("status").and_then(Value::as_str)
            == Some("python_backend_removal_execution_contract_ready")
        && python_removal
            .get("python_backend_removal_candidate")
            .and_then(Value::as_bool)
            == Some(true)
        && python_removal
            .get("python_backend_removed")
    ...
    if let Some(obj) = full_removal_rehearsal_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_REMOVAL_REHEARSAL"),
        );
        obj.insert(
            "python_backend_removal_execution_contract".to_string(),
            json!(python_removal.clone()),
        );
        obj.insert(
            "operator_full_rust_backend_removal_rehearsal_ack".to_string(),
            json!(true),
        );
        obj.insert(
            "rollback_path".to_string(),
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_removal_rehearsal_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_removal_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_mode".to_string(),
                json!("rehearsal_only"),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_removal_execution_contract"
                    .to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_python_fallback".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_require_no_side_effects".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_removal_rehearsal_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (full_removal_rehearsal, full_removal_rehearsal_errors, _full_removal_rehearsal_warnings) =
        build_full_rust_backend_removal_rehearsal_payload(&full_removal_rehearsal_payload);
    let full_removal_rehearsal_ok = full_removal_rehearsal_errors.is_empty()
        && full_removal_rehearsal.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_removal_rehearsal_ready")
        && full_removal_rehearsal
            .get("python_backend_removed")
            .and_then(Value::as_bool)
            == Some(false)
        && full_removal_rehearsal
    ...
    if let Some(obj) = full_production_cutover_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_PRODUCTION_CUTOVER"),
        );
        obj.insert(
            "full_rust_backend_removal_rehearsal".to_string(),
            json!(full_removal_rehearsal.clone()),
        );
        obj.insert(
            "operator_full_rust_backend_production_cutover_ack".to_string(),
            json!(true),
        );
        ...
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_production_cutover_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_production_cutover".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_mode".to_string(),
                json!("operator_supervised"),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_removal_rehearsal".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_rollback_path".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_require_python_fallback_backup".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_cutover_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        full_production_cutover,
        full_production_cutover_errors,
        _full_production_cutover_warnings,
    ) = build_full_rust_backend_production_cutover_payload(&full_production_cutover_payload);
    let full_production_cutover_ok = full_production_cutover_errors.is_empty()
        && full_production_cutover
            .get("status")
            .and_then(Value::as_str)
            == Some("full_rust_backend_production_cutover_ready")
        && full_production_cutover
            .get("cutover_allowed")
            .and_then(Value::as_bool)
            == Some(true)
    ...
    if let Some(obj) = full_production_verifier_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_PRODUCTION_VERIFIER"),
        );
        obj.insert(
            "full_rust_backend_production_cutover".to_string(),
            json!(full_production_cutover.clone()),
        );
        ...
        obj.insert(
            "python_backend_stopped_or_disabled".to_string(),
            json!(true),
        );
        obj.insert("production_healthcheck_passed".to_string(), json!(true));
        obj.insert(
            "operator_full_rust_backend_production_verifier_ack".to_string(),
            json!(true),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_production_verifier_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_production_verifier".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_mode".to_string(),
                json!("verify_only"),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_cutover".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_runtime_health".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_rollback_package".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_verifier_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (
        full_production_verifier,
        full_production_verifier_errors,
        _full_production_verifier_warnings,
    ) = build_full_rust_backend_production_verifier_payload(&full_production_verifier_payload);
    let full_production_verifier_ok = full_production_verifier_errors.is_empty()
        && full_production_verifier
            .get("status")
            .and_then(Value::as_str)
            == Some("full_rust_backend_production_verified")
        && full_production_verifier
            .get("full_rust_backend")
            .and_then(Value::as_bool)
            == Some(true)
        && full_production_verifier
            .get("python_retirement_executor_allowed")
            .and_then(Value::as_bool)
            == Some(true)
        && full_production_verifier
            .get("webui_ux_unchanged")
    ...
    if let Some(obj) = post_retirement_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_POST_RETIREMENT_VERIFIER"),
        );
        obj.insert(
            "full_rust_backend_production_verifier".to_string(),
            json!(full_production_verifier.clone()),
        );
        obj.insert(
            "rust_service_runtime_authoritative".to_string(),
            json!(true),
        );
        obj.insert(
            "python_backend_service_masked_or_disabled".to_string(),
            json!(true),
        );
        obj.insert(
            "python_backend_files_preserved_for_rollback".to_string(),
            json!(true),
        );
        obj.insert("python_api_routes_unregistered".to_string(), json!(true));
        obj.insert(
            "post_retirement_healthcheck_passed".to_string(),
            json!(true),
        );
        obj.insert(
            "operator_full_rust_backend_post_retirement_ack".to_string(),
            json!(true),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_post_retirement_verifier_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_post_retirement_verifier".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_verifier_mode".to_string(),
                json!("verify_only"),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_production_verifier".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_runtime_health".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_python_retired".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_rollback_package".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_post_retirement_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (post_retirement, post_retirement_errors, _post_retirement_warnings) =
        build_full_rust_backend_post_retirement_verifier_payload(&post_retirement_payload);
    let post_retirement_ok = post_retirement_errors.is_empty()
        && post_retirement.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_post_retirement_verified")
        && post_retirement
            .get("full_rust_backend")
            .and_then(Value::as_bool)
            == Some(true)
    ...
    if let Some(obj) = steady_state_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_STEADY_STATE_GUARD"),
        );
        obj.insert(
            "full_rust_backend_post_retirement_verifier".to_string(),
            json!(post_retirement.clone()),
        );
        obj.insert("steady_state_healthcheck_passed".to_string(), json!(true));
        obj.insert(
            "python_backend_unexpectedly_running".to_string(),
            json!(false),
        );
        ...
        obj.insert(
            "python_backend_rollback_package_ready".to_string(),
            json!(true),
        );
        obj.insert(
            "operator_full_rust_backend_steady_state_ack".to_string(),
            json!(true),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_steady_state_guard_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_steady_state_guard".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_guard_mode".to_string(),
                json!("guard_only"),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_post_retirement_verifier".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_runtime_health".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_no_python_drift".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_rollback_package".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_steady_state_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (steady_state, steady_state_errors, _steady_state_warnings) =
        build_full_rust_backend_steady_state_guard_payload(&steady_state_payload);
    let steady_state_ok = steady_state_errors.is_empty()
        && steady_state.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_steady_state_verified")
        && steady_state
            .get("full_rust_backend")
            .and_then(Value::as_bool)
            == Some(true)
        && steady_state
            .get("python_drift_absent")
            .and_then(Value::as_bool)
            == Some(true);
    checks.push(check(
    ...
    if let Some(obj) = drift_monitor_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_PRODUCTION_DRIFT_MONITOR"),
        );
        obj.insert(
            "full_rust_backend_steady_state_guard".to_string(),
            json!(steady_state.clone()),
        );
        ...
        obj.insert(
            "operator_full_rust_backend_drift_monitor_ack".to_string(),
            json!(true),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_production_drift_monitor_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_production_drift_monitor".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_drift_monitor_mode".to_string(),
                json!("monitor_only"),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_steady_state_guard".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_runtime_health".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_no_python_drift".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_rollback_package".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_drift_monitor_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (drift_monitor, drift_monitor_errors, _drift_monitor_warnings) =
        build_full_rust_backend_production_drift_monitor_payload(&drift_monitor_payload);
    let drift_monitor_ok = drift_monitor_errors.is_empty()
        && drift_monitor.get("status").and_then(Value::as_str)
            == Some("full_rust_backend_production_drift_monitor_healthy")
        && drift_monitor
            .get("full_rust_backend")
            .and_then(Value::as_bool)
            == Some(true)
    ...
    if let Some(obj) = audit_sentinel_payload.as_object_mut() {
        obj.insert(
            "confirmation".to_string(),
            json!("CONFIRM_FULL_RUST_BACKEND_PRODUCTION_AUDIT_SENTINEL"),
        );
        obj.insert(
            "full_rust_backend_production_drift_monitor".to_string(),
            json!(drift_monitor.clone()),
        );
        ...
        obj.insert(
            "transaction_journal_preview_passed".to_string(),
            json!(true),
        );
        obj.insert(
            "transaction_journal_redaction_verified".to_string(),
            json!(true),
        );
        obj.insert("transaction_journal_entry_count".to_string(), json!(1));
        obj.insert(
            "rollback_manifest_preview_available".to_string(),
            json!(true),
        );
        obj.insert(
            "rollback_from_journal_preview_available".to_string(),
            json!(true),
        );
        obj.insert("audit_sentinel_healthcheck_passed".to_string(), json!(true));
        obj.insert(
            "operator_full_rust_backend_audit_sentinel_ack".to_string(),
            json!(true),
        );
        if let Some(rc) = obj.get_mut("rust_core").and_then(Value::as_object_mut) {
            rc.insert(
                "full_rust_backend_production_audit_sentinel_pilot".to_string(),
                json!(true),
            );
            rc.insert(
                "allow_full_rust_backend_production_audit_sentinel".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_production_audit_sentinel_mode".to_string(),
                json!("sentinel_only"),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_drift_monitor".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_audit_trail".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_journal_visibility".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_rollback_visibility".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_webui_unchanged".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_server_tests".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_manual_confirmation".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_require_operator_ack".to_string(),
                json!(true),
            );
            rc.insert(
                "full_rust_backend_audit_sentinel_max_shadow_age_seconds".to_string(),
                json!(900),
            );
        }
    }
    let (audit_sentinel, audit_sentinel_errors, _audit_sentinel_warnings) =
        build_full_rust_backend_production_audit_sentinel_payload(&audit_sentinel_payload);
    let audit_sentinel_ok = audit_sentinel_errors.is_empty()
        && audit_sentinel.get("status").and_then(Value::as_str)
    ...
    let (collector_bundle, collector_bundle_errors, _collector_bundle_warnings) =
        build_collector_circuit_bundle_payload(&collector_bundle_payload);
    let collector_bundle_ok = collector_bundle_errors.is_empty()
        && collector_bundle
            .get("normalized_count")
            .and_then(Value::as_u64)
            .unwrap_or(0)
    ...
    if !collector_bundle_ok {
        errors.push(Diagnostic::error(
            "self_test_collector_bundle_failed",
            Some("build-collector-circuit-bundle".to_string()),
            "Self-test collector bundle should build one PPP circuit row in shadow mode.",
        ));
    }

    let collector_rows = collector_bundle
        .get("normalized_rows")
        .cloned()
        .unwrap_or_else(|| json!([]));
    let (collector_parity, collector_parity_errors, _collector_parity_warnings) =
    ...
    if !collector_parity_ok {
        errors.push(Diagnostic::error(
            "self_test_collector_parity_failed",
            Some("compare-collector-bundle-parity".to_string()),
            "Self-test collector parity should pass when Python and Rust rows are identical.",
        ));
    }
    ...
    let (shadow_bundle, shadow_bundle_errors, _shadow_bundle_warnings) =
        build_routeros_shadow_collector_bundle_payload(&shadow_bundle_payload);
    let shadow_bundle_ok = shadow_bundle_errors.is_empty()
        ...
        && shadow_bundle
            ...
            == 1
        && shadow_bundle
            .get("python_authoritative")
            .and_then(Value::as_bool)
            == Some(true);
    checks.push(check(
        "routeros_shadow_collector_bundle",
        shadow_bundle_ok,
        json!({
    ...
    let (manifest, manifest_errors, manifest_warnings) =
        build_apply_manifest_payload(&manifest_payload);
    let manifest_ok = manifest_errors.is_empty()
        && manifest.get("status").and_then(Value::as_str).unwrap_or("") == "no_changes";
    checks.push(check(
        "apply_manifest_no_changes",
        manifest_ok,
        json!({"status": manifest.get("status"), "warning_count": manifest_warnings.len()}),
    ));
    if !manifest_ok {
        errors.push(Diagnostic::error(
            "self_test_apply_manifest_failed",
            Some("build-apply-manifest".to_string()),
            "Self-test no-changes apply manifest did not produce expected status.",
        ));
    }
    ...
    let tx_ok = tx_errors.is_empty()
        && tx.get("executed").and_then(Value::as_bool).unwrap_or(true) == false;
    checks.push(check(
        "apply_transaction_rehearsal",
        tx_ok,
        json!({"status": tx.get("status"), "executed": tx.get("executed")}),
    ));
    if !tx_ok {
        errors.push(Diagnostic::error(
            "self_test_apply_transaction_failed",
            Some("execute-apply-transaction".to_string()),
            "Self-test transaction rehearsal should not execute writes.",
        ));
    }
    ...
    let (journal, journal_errors, _journal_warnings) =
        build_transaction_journal_payload(&journal_payload);
    let journal_ok = journal_errors.is_empty()
        && journal
            .get("append_executed")
            .and_then(Value::as_bool)
            .unwrap_or(true)
    ...
    if !journal_ok {
        errors.push(Diagnostic::error(
            "self_test_transaction_journal_failed",
            Some("build-transaction-journal".to_string()),
            "Self-test transaction journal preview should be non-mutating and valid.",
        ));
    }
    ...
    let (journal_append, journal_append_errors, _journal_append_warnings) =
        append_transaction_journal_payload(&append_payload);
    let journal_append_ok = journal_append_errors.is_empty()
        && journal_append
            .get("append_executed")
            .and_then(Value::as_bool)
            .unwrap_or(true)
    ...
    if !journal_append_ok {
        errors.push(Diagnostic::error(
            "self_test_transaction_journal_append_failed",
            Some("append-transaction-journal".to_string()),
            "Self-test transaction journal append rehearsal should not write.",
        ));
    }

    let (rollback, rollback_errors, _rollback_warnings) =
        build_rollback_manifest_payload(&journal_payload);
    let rollback_ok = rollback_errors.is_empty()
        && rollback
    ...
    if !rollback_ok {
        errors.push(Diagnostic::error(
            "self_test_rollback_manifest_failed",
            Some("build-rollback-manifest".to_string()),
            "Self-test rollback manifest should be preview-only and valid.",
        ));
    }

    let journal_event = journal.get("event").cloned().unwrap_or_else(|| json!({}));
    let temp_journal_path = std::env::temp_dir().join(format!(
        "lqosync-core-self-test-journal-{}.jsonl",
        std::process::id()
    ));
    let _ = std::fs::write(
        &temp_journal_path,
    ...
    let (history, history_errors, _history_warnings) =
        read_transaction_journal_payload(&json!({"path": temp_journal, "limit": 1}));
    let history_ok = history_errors.is_empty()
        && history
            .get("returned_count")
            .and_then(Value::as_u64)
            .unwrap_or(0)
            >= 1;
    checks.push(check(
        "transaction_journal_reader",
        history_ok,
        json!({"returned_count": history.get("returned_count"), "status": history.get("status")}),
    ));
    if !history_ok {
        errors.push(Diagnostic::error(
            "self_test_transaction_journal_reader_failed",
            Some("read-transaction-journal".to_string()),
            "Self-test transaction journal reader should read a temporary JSONL entry.",
        ));
    }

    let journal_id = journal_event
        .get("journal_id")
        .and_then(Value::as_str)
        .unwrap_or("");
    let (rollback_from_journal, rollback_from_journal_errors, _rollback_from_journal_warnings) =
        build_rollback_from_journal_payload(
            &json!({"path": temp_journal_path.to_string_lossy(), "journal_id": journal_id}),
        );
    let rollback_from_journal_ok = rollback_from_journal_errors.is_empty()
    ...
    if !rollback_from_journal_ok {
        errors.push(Diagnostic::error(
            "self_test_rollback_from_journal_failed",
            Some("build-rollback-from-journal".to_string()),
            "Self-test rollback-from-journal should build a preview from a temporary JSONL entry.",
        ));
    }

    let (rollback_execute, rollback_execute_errors, _rollback_execute_warnings) =
        execute_rollback_payload(&json!({
            "rollback_manifest": rollback_from_journal.clone(),
            "execute": false,
            "allow_rollback_file_writes": false
        }));
    let rollback_execute_ok = rollback_execute_errors.is_empty()
        && rollback_execute
            .get("executed")
            .and_then(Value::as_bool)
            .unwrap_or(true)
    ...
    if !rollback_execute_ok {
        errors.push(Diagnostic::error(
            "self_test_rollback_execute_failed",
            Some("execute-rollback".to_string()),
            "Self-test rollback execution rehearsal should not restore files.",
        ));
    }
    ...
    let authority_readiness_ok = authority_readiness_errors.is_empty()
        && authority_readiness
            .get("verdict")
            .and_then(Value::as_str)
            .unwrap_or("")
    ...
    let (pilot_plan, pilot_plan_errors, _pilot_plan_warnings) =
        build_authority_pilot_plan_payload(&json!({
    ...
    if !pilot_plan_ok {
        errors.push(Diagnostic::error(
            "self_test_authority_pilot_plan_failed",
            Some("build-authority-pilot-plan".to_string()),
            "Self-test authority pilot plan should produce ordered non-mutating stages.",
        ));
    }
