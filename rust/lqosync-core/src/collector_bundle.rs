...
fn speed_from_text(raw: &str, parser: &str) -> Option<BandwidthPair> {
    ...
    if parser == "rate_limit" {
        ...
        if p.download_mbps > 0.0 && p.upload_mbps > 0.0 {
            Some(p)
        } else {
            None
        }
...
fn resolve_pppoe_speed(
    secret: &Value,
    active: &Value,
    profile: Option<&Value>,
    profile_name: &str,
    default_rate: &str,
) -> (BandwidthPair, String, String) {
    ...
    let candidates = [
        ...
        (
            sval(profile, "rate-limit"),
            "ppp_profile_rate_limit",
            "rate_limit",
        ),
        (
            default_rate.to_string(),
            "config_default_pppoe",
            "rate_limit",
        ),
    ];
    ...
    (
        ...
        "config_default_pppoe_fallback".to_string(),
        default_rate.to_string(),
    )
}
...
fn make_row(
    ...
    source: &str,
    pair: &BandwidthPair,
    min_rate_percentage: f64,
) -> ShapedDeviceRow {
    let mut fields = BTreeMap::new();
    for f in FIELDNAMES {
        fields.insert(f.to_string(), String::new());
    }
    ...
    fields.insert(
        "Download Min Mbps".to_string(),
        round3(pair.download_mbps * min_rate_percentage),
    );
    fields.insert(
        "Upload Min Mbps".to_string(),
        round3(pair.upload_mbps * min_rate_percentage),
    );
...
fn row_json(row: &ShapedDeviceRow) -> Value {
    json!(row.fields)
}
...
fn process_pppoe(
    ...
    router: &Value,
    defaults: &Value,
    rows: &mut Vec<ShapedDeviceRow>,
    warnings: &mut Vec<Diagnostic>,
    meta: &mut Vec<Value>,
    min_rate_percentage: f64,
) -> usize {
    let Some(pppoe) = obj(payload, "pppoe") else {
        return 0;
    };
    let router_name = sval(router, "name");
    let router_name = if router_name.is_empty() {
        "Router".to_string()
    } else {
        router_name
    };
    let default_rate = sval(defaults, "default_pppoe_rate");
    let default_rate = if default_rate.is_empty() {
        "10M/10M".to_string()
    } else {
        default_rate
    };
    ...
    let plan_tpl = if plan_tpl.is_empty() {
        "{profile}-{router}".to_string()
    } else {
        plan_tpl
    };
    let flat_tpl = sval(router_pppoe, "flat_node_name");
    let flat_tpl = if flat_tpl.is_empty() {
        "PPP-{router}".to_string()
    } else {
        flat_tpl
    };
    ...
    for a in active_rows {
        let name = sval(a, "name");
        if !name.is_empty() {
            active_by_name.insert(name, a);
        }
    }
    for s in secret_rows {
        let name = sval(s, "name");
        if !name.is_empty() && !boolish(s, "disabled") && !boolish(s, "inactive") {
            secret_by_name.insert(name, s);
        }
    }
    for p in profile_rows {
        let name = sval(p, "name");
        if !name.is_empty() {
            profile_by_name.insert(name, p);
        }
    }
    ...
    for (username, active) in active_by_name.iter() {
        ...
        let (pair, speed_source, raw_value) = resolve_pppoe_speed(
            secret,
            active,
            profile_by_name.get(&profile_name).copied(),
            &profile_name,
            &default_rate,
        );
        let code = if !username.trim().is_empty() {
            username.to_string()
        } else {
            clean_mac(&mac)
        };
        ...
        let parent = if per_plan {
            replace_template(&plan_tpl, &router_name, &profile_name, "", "")
        } else {
            replace_template(&flat_tpl, &router_name, "", "", "")
        };
        rows.push(make_row(
            &code,
            username,
        ...
        meta.push(json!({
            ...
            "base_download_mbps": pair.download_mbps,
            "base_upload_mbps": pair.upload_mbps,
            "speed_source": speed_source,
            "speed_raw_value": raw_value
        }));
        count += 1;
    }
    count
...
fn process_dhcp(
    ...
    let Some(dhcp) = obj(payload, "dhcp") else {
        return 0;
    };
    let router_name = sval(router, "name");
    let router_name = if router_name.is_empty() {
    ...
    if let Some(servers) = dhcp.get("servers").and_then(Value::as_array) {
        for server in servers {
            let name = sval(server, "name");
            if !name.is_empty() {
                server_by_name.insert(name, server);
            }
        }
    }
    ...
    for lease in arr(dhcp, "leases") {
        ...
        let code = if !hostname.trim().is_empty() {
            format!("DHCP-{hostname}")
        } else {
            format!("DHCP-{mac_clean}")
        };
        if code == "DHCP-" {
            warnings.push(Diagnostic::warning(
                "dhcp_lease_missing_identity",
                Some("dhcp.leases".to_string()),
                "DHCP lease missing hostname and MAC; skipped.",
            ));
            continue;
        }
        let down = f64ish(server, "default_plan_down_mbps")
            .or_else(|| f64ish(server, "download_limit_mbps"))
            .unwrap_or(default_mbps);
        let up = f64ish(server, "default_plan_up_mbps")
            .or_else(|| f64ish(server, "upload_limit_mbps"))
            .unwrap_or(default_mbps);
        let tpl = sval(server, "node_name");
        let tpl = if tpl.is_empty() {
            "DHCP-{server}-{router}".to_string()
        } else {
            tpl
        };
        ...
        meta.push(json!({
            ...
            "server": server_name,
            "hostname": hostname,
            "base_download_mbps": down,
            "base_upload_mbps": up,
            "speed_source": "dhcp_server_config"
        }));
        count += 1;
    }
    count
...
fn process_hotspot(
    ...
    let Some(hs) = obj(payload, "hotspot") else {
        return 0;
    };
    let router_name = sval(router, "name");
    let router_name = if router_name.is_empty() {
    ...
    for active in arr(hs, "active") {
        ...
        let ip = sval(active, "address");
        let mac_clean = clean_mac(&mac);
        let code = if include_mac && !mac_clean.is_empty() {
            format!("HS-{mac_clean}")
        ...
        if code.is_empty() {
            warnings.push(Diagnostic::warning(
                "hotspot_active_missing_identity",
                Some("hotspot.active".to_string()),
                "Hotspot active row missing username and MAC; skipped.",
            ));
            continue;
        }
        rows.push(make_row(
        ...
        meta.push(json!({
            ...
            "base_download_mbps": down,
            "base_upload_mbps": up,
            "speed_source": "hotspot_config_speed"
        }));
        count += 1;
    }
    count
...
pub fn build_collector_circuit_bundle_payload(
    payload: &Value,
    ...
    let min_rate_percentage = f64ish(defaults, "min_rate_percentage")
        .or_else(|| f64ish(payload, "min_rate_percentage"))
        .unwrap_or(0.5)
        .clamp(0.0, 1.0);
    ...
    if rows.is_empty() {
        warnings.push(Diagnostic::warning(
            "collector_bundle_empty",
            Some("collector_bundle".to_string()),
            "Rust collector bundle produced zero circuit rows.",
        ));
    }
...
mod tests {
    ...
    fn warns_on_pppoe_active_without_secret() {
        ...
        assert!(warnings
            .iter()
            .any(|w| w.code == "pppoe_active_without_secret"));
    }
